...
export async function consumirDaFila(): Promise<string> {
  try {
    const textoDoArquivo = await leArquivo()
    console.log('texto encontrado anteriormente no arquivo', textoDoArquivo);
    const [linhaConsumida, ...linhas] = textoDoArquivo.split('\n');
